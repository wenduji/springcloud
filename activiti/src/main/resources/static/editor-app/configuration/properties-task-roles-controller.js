...
var KisBpmTaskRolesPopupCtrl = [ '$scope', '$q', '$translate', function($scope, $q, $translate) {
    ...
    if ($scope.property.value !== undefined && $scope.property.value !== null
        ...
        if ($scope.property.value.taskRoles.constructor == String)
        {
            $scope.taskRoles = JSON.parse($scope.property.value.taskRoles);
        }
        else
        {
        ...
        {
            var taskListener = $scope.taskRoles[i];
    ...
    $scope.addNewListener = function() {
        $scope.taskRoles.push({ event : 'create',
            implementation : '',
            className : '',
            expression: '',
            delegateExpression: ''});
    ...
    $scope.removeListener = function() {
        if ($scope.selectedListeners.length > 0) {
            var index = $scope.taskRoles.indexOf($scope.selectedListeners[0]);
